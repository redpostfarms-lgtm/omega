...
if (Test-Path $venvPip) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "      [OK] pythonnet installed successfully`n" -ForegroundColor Green
    }
    else {
        Write-Host "      [ERROR] pythonnet installation failed`n" -ForegroundColor Red
    }
}
else {
    Write-Host "      [ERROR] Virtual environment not found`n" -ForegroundColor Red
}
...
if (Test-Path $dllPath) {
    Write-Host "      [OK] Already installed at: $installDir" -ForegroundColor Green
