...
async function initializeOmega(context) {
    const config = vscode.workspace.getConfiguration('omega');
    ...
    const interval = config.get('optimizationInterval', 20) * 60 * 1000;
...
async function loadBrainCore() {
    try {
        ...
        const brainConfig = {
            gpuPriority: vscode.workspace.getConfiguration('omega').get('gpuPriority', 60),
            cpuPriority: vscode.workspace.getConfiguration('omega').get('cpuPriority', 40),
            ramCacheLimit: vscode.workspace.getConfiguration('omega').get('ramCacheLimit', 512),
            memoryThreshold: vscode.workspace.getConfiguration('omega').get('memoryThreshold', 85)
        };
...
async function runOptimization() {
    ...
    const scriptPath = config.get('optimizerScript', '');
...
function registerCommands(context) {
    // Optimize command
    context.subscriptions.push(vscode.commands.registerCommand('omega.optimize', async () => {
    ...
    // Status command
    context.subscriptions.push(vscode.commands.registerCommand('omega.status', async () => {
        const mem = await si.mem();
        const cpu = await si.currentLoad();
        const config = vscode.workspace.getConfiguration('omega');
...
Autocomplete: ${config.get('enableAutocomplete') ? '✅ Enabled' : '❌ Disabled'}
    ...
    // Configure command
    context.subscriptions.push(vscode.commands.registerCommand('omega.configure', async () => {
        const config = vscode.workspace.getConfiguration('omega');
    ...
    // Load brain command
    context.subscriptions.push(vscode.commands.registerCommand('omega.loadBrain', async () => {
        await vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
 ...
 */
function activateAutocomplete(context) {
    console.log('🤖 Activating Omega Predictive Autocomplete...');
    // Register completion provider for all languages
    completionProvider = vscode.languages.registerCompletionItemProvider({ scheme: 'file', language: '*' }, {
        provideCompletionItems(document, position) {
            ...
            predictions.forEach((prediction, index) => {
                const completion = new vscode.CompletionItem(prediction.text, vscode.CompletionItemKind.Snippet);
...
function analyzeContext(document, position) {
    ...
    for (let i = position.line - 1; i >= 0; i--) {
        ...
        if (functionContext && classContext)
            break;
    }
    return {
        language,
...
function generatePredictions(context, linePrefix) {
    const predictions = [];
