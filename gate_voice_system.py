...
class GateVoiceSystem:
    ...
    def __init__(self):
        ...
        # KITT voice file path - Official authentic KITT voice (KIT1.wav)
        self.kitt_voice_path = Path("H:/The Gatekeeper/gate_kitt_voice.wav")
    ...
    def speak(self, text, save_to=None):
        ...
        try:
            # Generate speech using KITT voice with HIGH-QUALITY settings
            output_path = save_to or "gate_speech_output.wav"

            # ADVANCED SETTINGS for 98th percentile voice matching
            self.tts.tts_to_file(
                ...
                temperature=0.65,  # Lower = more consistent with source (default 0.85)
                length_penalty=1.0,  # Maintain natural pacing
            ...
            # Load audio - NO pitch adjustment to preserve authentic KITT voice
            data, samplerate = sf.read(output_path)

            # Play the authentic voice (no modifications)
            print("▶️ Playing with 98% voice accuracy matching...")
