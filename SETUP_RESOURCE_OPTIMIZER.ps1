...
$action = New-ScheduledTaskAction `
    -Execute $pythonPath `
    -Argument "`"$scriptPath`"" `
    -WorkingDirectory "H:\The Gatekeeper"
...
Register-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "Omega Resource Optimizer - Enforces GPU/CPU priority, keeps memory below 85%" | Out-Null

Write-Host "✅ Scheduled task created successfully!" -ForegroundColor Green
Write-Host ""

# Run initial optimization
Write-Host "Running initial optimization..." -ForegroundColor Cyan
Write-Host ""
